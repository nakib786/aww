...
interface GlowCardProps {
  ...
  mobileMode?: 'rotate' | 'pulse' | 'breathe' | 'colorShift' | 'flowHorizontal' | 'static';
}
...
const mobileColorMap = {
  blue: ['#0894FF', '#00D4FF', '#0066FF', '#4A90E2'],
...
const GlowCard: React.FC<GlowCardProps> = ({ 
  ...
  return (
    <>
      {isHydrated && !isTouchDevice && <style dangerouslySetInnerHTML={{ __html: beforeAfterStyles }} />}
      <div
        ref={cardRef}
        data-glow={isHydrated && !isTouchDevice}
        style={isHydrated && isTouchDevice ? {} : getInlineStyles()}
        className={`
          ${getSizeClasses()}
          ...
          backdrop-blur-[5px]
          overflow-hidden
          ${!isHydrated ? 'bg-black/20 border border-white/10' : ''}
          ${className}
        `}
      >
        {isHydrated && isTouchDevice && (
          <GlowEffect
            colors={mobileColorMap[glowColor]}
            mode={mobileMode}
            blur="medium"
            scale={1.2}
            duration={4}
        ...
        {isHydrated && !isTouchDevice && <div ref={innerRef} data-glow></div>}
        {!isHydrated && (
